...
export default function PreviewBook({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "var(--sage)" }}>
        <div style={{ color: "var(--midnight)" }}>Loading preview...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "var(--sage)" }}>
      {/* Top bar */}
      <header className="sticky top-0 z-10 py-4 px-6 border-b flex justify-between items-center" style={{ backgroundColor: "rgba(192,201,153,0.9)", borderColor: "var(--thistle)" }}>
        <div className="flex items-center gap-3">
          <Link href={`/books/${id}`} className="text-sm transition-colors flex items-center gap-1" style={{ color: "var(--midnight)" }}>
            <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6"/></svg>
            Back
          </Link>
          <span style={{ color: "var(--rosy)" }}>·</span>
          <span className="text-sm" style={{ color: "var(--midnight)" }}>{book.title}</span>
        </div>
        <button
          onClick={handleOrderPrint}
          disabled={ordering || memories.length === 0}
          className="inline-flex h-8 shrink-0 items-center justify-center rounded-lg px-3 py-2 text-sm font-medium whitespace-nowrap transition-opacity disabled:opacity-50"
      ...
      <main className="flex-1 py-10 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--white)", boxShadow: "0 25px 50px -12px rgba(44,19,32,0.15)", border: "1px solid var(--thistle)" }}>
            ...
            <div className="flex">
              ...
              <div className="flex-1">
                ...
                <div className="px-12 py-16 text-center border-b-4" style={{ background: "linear-gradient(to bottom, rgba(196,183,203,0.15), rgba(196,183,203,0.05), var(--white))", borderColor: "var(--rosy)" }}>
                  ...
                  <div className="border-t border-b py-6 mb-6" style={{ borderColor: "rgba(71,75,36,0.3)" }}>
                ...
                {/* Table of contents */}
                {memories.length > 0 && (
                  <div className="px-12 py-10 border-b" style={{ borderColor: "var(--thistle)" }}>
                    <h3 className="text-xs font-bold uppercase tracking-widest mb-6" style={{ color: "var(--rosy)" }}>Contents</h3>
                ...
                {memories.map((memory, i) => (
                  <div key={memory.id} className="px-12 py-10 border-b last:border-b-0" style={{ borderColor: "var(--thistle)" }}>
                    {/* Memory header */}
                    <div className="mb-6">
          ...
          {memories.length === 0 && (
            <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--white)", boxShadow: "0 25px 50px -12px rgba(44,19,32,0.15)", border: "1px solid var(--thistle)" }}>
              <div className="px-12 py-20 text-center">
                <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: "var(--sage)" }}>
                  <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: "var(--midnight)" }}>
                    <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/>
                    <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
                  </svg>
                </div>
                <h2 className="text-lg font-semibold mb-2" style={{ fontFamily: "var(--font-serif), 'Lora', Georgia, serif", color: "var(--midnight)" }}>Nothing to preview yet</h2>
                <p className="text-sm mb-6" style={{ color: "var(--rosy)" }}>Add some memories to your book before previewing.</p>
                <Link
                  href={`/books/${id}/edit`}
          ...
          <p className="text-center text-xs mt-6" style={{ color: "var(--rosy)", opacity: 0.6 }}>
