...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: 'var(--cornsilk)' }}>
                  ...
                  <form onSubmit={handlePasswordLogin} className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        />
                        <button type="button" onClick={() => setShowPassword(p => !p)} aria-label={showPassword ? 'Hide password' : 'Show password'} aria-pressed={showPassword} className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center rounded-full transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--bronze)] focus-visible:ring-offset-1 hover:opacity-80 active:opacity-70" style={{ color: 'rgba(212,163,115,0.65)' }}>
      ...
      {/* Mobile nav drawer */}
      <MobileNav id="mobile-nav" isOpen={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)} loggedIn={false} />
    </div>
  );
}
