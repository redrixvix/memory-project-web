...
export default function LoginClient() {
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setPasswordError(data.error || 'Unable to sign in with that email and password.');
        setError('');
        return;
      }
                ...
                <div className="space-y-5">
                  ...
                  <form onSubmit={handlePasswordLogin} className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <Input
                          ...
                          style={{ borderColor: passwordError ? 'rgba(180,60,60,0.5)' : 'rgba(212,163,115,0.3)', backgroundColor: '#FFFDF8' }}
                        />
                        <button type="button" onClick={() => setShowPassword(p => !p)} aria-label={showPassword ? 'Hide password' : 'Show password'} aria-pressed={showPassword} className="absolute right-3 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center rounded-xl transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--bronze)] focus-visible:ring-offset-1 hover:opacity-80 active:opacity-70" style={{ color: 'rgba(212,163,115,0.65)' }}>
                      ...
                      {passwordError && (
                        <p role="alert" id="password-error" className="text-xs mt-1.5" style={{ color: 'rgba(180,60,60,0.85)' }}>
                          {passwordError}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      <span className="text-xs" style={{ color: 'rgba(43,43,43,0.55)', fontFamily: 'var(--font-sans)' }}>Your memories are private and encrypted</span>
                    </div>
                  </form>
